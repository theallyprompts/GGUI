...
import { useIllustriousStore, MAX_LORAS } from '../../../store/models/illustrious.store'
...
import { AcceleratorSettings } from '../z-image/AcceleratorSettings'
import { LoraSettings } from '../z-image/LoraSettings'
import { VaeInput } from './VaeInput'

const OUTPUT_FORMATS = ['JPG', 'PNG', 'WEBP'] as const

export function IllustriousForm({ model }: { model: ImageInferenceModelDefinition }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const acceleratorOptions =
      accelerator.fbCache || accelerator.teaCache || accelerator.dbCache
        ? {
            ...(accelerator.fbCache && {
              fbCache: true,
              fbCacheThreshold: accelerator.fbCacheThreshold,
            }),
            ...(accelerator.teaCache && {
              teaCache: true,
              teaCacheDistance: accelerator.teaCacheDistance,
            }),
            ...(accelerator.dbCache && {
              dbCache: true,
              dbCacheThreshold: accelerator.dbCacheThreshold,
            }),
          }
        : undefined
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
        ...
        {showAdvanced && (
          <div className="flex flex-col gap-4 rounded-md border border-neutral-70 bg-card p-3">
            ...
            <button
              type="button"
              onClick={() => setShowAccelerator((v) => !v)}
              className="flex w-full items-center justify-between rounded-md border border-neutral-70 bg-input px-3 py-2 text-xs font-medium text-neutral-20"
            >
              Accelerator
              <svg
                viewBox="0 0 24 24"
                ...
                stroke="currentColor"
                strokeWidth="2"
                className={`h-3.5 w-3.5 text-neutral-40 transition-transform ${showAccelerator ? 'rotate-180' : ''}`}
              >
                <path strokeLinecap="round" strokeLinejoin="round" d="m6 9 6 6 6-6" />
              </svg>
            </button>
            {showAccelerator && (
              <AcceleratorSettings value={accelerator} onChange={setAccelerator} />
            )}
          </div>
        )}
      </div>
