...
import { useFluxDevStore } from '../../../store/models/fluxDev.store'

export function FluxDevPricingInfo({ model }: { model: ImageInferenceModelDefinition }) {
  const width = useFluxDevStore((s) => s.width)
  const height = useFluxDevStore((s) => s.height)
