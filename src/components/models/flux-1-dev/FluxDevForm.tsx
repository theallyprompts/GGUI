...
import { useFluxDevStore, MAX_LORAS } from '../../../store/models/fluxDev.store'
...
import { FluxAcceleratorSettings } from './FluxAcceleratorSettings'

const OUTPUT_FORMATS = ['JPG', 'PNG', 'WEBP'] as const

export function FluxDevForm({ model }: { model: ImageInferenceModelDefinition }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const acceleratorOptions =
      accelerator.teaCache || accelerator.dbCache
        ? {
            ...
            ...(accelerator.dbCache && {
              ...
              dbCacheSkipInterval: accelerator.dbCacheSkipInterval,
            }),
          }
        : undefined
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
        ...
        {showAdvanced && (
          <div className="flex flex-col gap-4 rounded-md border border-neutral-70 bg-card p-3">
            ...
            {showAccelerator && (
              <FluxAcceleratorSettings value={accelerator} onChange={setAccelerator} />
            )}
          </div>
        )}
      </div>
