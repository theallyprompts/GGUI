...
import { useFlux2KleinStore, MAX_LORAS } from '../../../store/models/flux2Klein.store'
import { useApiKeyStore } from '../../../store/apiKey.store'
import { SCHEDULERS, type AccelerationLevel, type Scheduler } from '../../../lib/runware/types'
...
const ACCELERATION_LEVELS: AccelerationLevel[] = ['none', 'low', 'medium', 'high']

export function Flux2KleinForm({ model }: { model: ImageInferenceModelDefinition }) {
    ...
    <form onSubmit={handleSubmit} className="flex h-full min-h-0 flex-col">
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
        ...
        <ReferenceMediaList
          ...
          max={model.maxReferenceImages ?? 4}
        ...
        {showAdvanced && (
          <div className="flex flex-col gap-4 rounded-md border border-neutral-70 bg-card p-3">
            <Select
              label="Acceleration"
              value={acceleration}
              options={ACCELERATION_LEVELS}
              onChange={setAcceleration}
              tooltip="Speeds up generation using caching and step-skipping optimizations. Higher levels generate faster but can reduce image quality."
            />
            <NumericSlider
              label="CFG Scale"
              value={cfgScale}
