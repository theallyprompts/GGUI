...
import type { Sd15RealisticVisionJob } from '../generation.store'

export const MAX_LORAS = 5

const DEFAULT_ASPECT_RATIO = SD15_REALISTIC_VISION_MODEL.aspectRatios[0]
const DEFAULT_RESOLUTION_TIER = SD15_REALISTIC_VISION_MODEL.resolutionTiers[0] // 512
...
interface Sd15RealisticVisionState {
  ...
  setField: <K extends 'positivePrompt' | 'negativePrompt' | 'seed' | 'vae'>(
    field: K,
    value: Sd15RealisticVisionState[K],
  ) => void
  ...
  setOutputFormat: (format: Sd15RealisticVisionState['outputFormat']) => void
  ...
  loadFromJob: (job: Sd15RealisticVisionJob, options?: { includeSeed?: boolean }) => void
  resetForm: () => void
}
...
const DEFAULTS = {
  ...
  steps: SD15_REALISTIC_VISION_MODEL.defaultSteps,
  cfgScale: SD15_REALISTIC_VISION_MODEL.defaultCFGScale,
...
export const useSd15RealisticVisionStore = create<Sd15RealisticVisionState>((set, get) => ({
  ...DEFAULTS,

  setField: (field, value) => set({ [field]: value } as never),
  setAspectRatio: (label) => {
    const ratio =
      SD15_REALISTIC_VISION_MODEL.aspectRatios.find((r) => r.label === label) ??
      SD15_REALISTIC_VISION_MODEL.aspectRatios[0]
    const tier =
      SD15_REALISTIC_VISION_MODEL.resolutionTiers.find((t) => t.label === get().resolutionTierLabel) ??
      SD15_REALISTIC_VISION_MODEL.resolutionTiers[0]
    const { width, height } = resolveDimensions(SD15_REALISTIC_VISION_MODEL, ratio.ratio, tier)
    set({ aspectRatioLabel: label, width, height })
  },
  setResolutionTier: (label) => {
    const tier =
      SD15_REALISTIC_VISION_MODEL.resolutionTiers.find((t) => t.label === label) ??
      SD15_REALISTIC_VISION_MODEL.resolutionTiers[0]
    const ratio =
      SD15_REALISTIC_VISION_MODEL.aspectRatios.find((r) => r.label === get().aspectRatioLabel) ??
      SD15_REALISTIC_VISION_MODEL.aspectRatios[0]
    const { width, height } = resolveDimensions(SD15_REALISTIC_VISION_MODEL, ratio.ratio, tier)
  ...
  loadFromJob: (job, options) => {
    const ratio = findClosestAspectRatio(SD15_REALISTIC_VISION_MODEL, job.width, job.height)
    const tier = findClosestResolutionTier(SD15_REALISTIC_VISION_MODEL, job.width, job.height)
    set({
