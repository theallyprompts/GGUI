...
import type { AutismmixPonyJob } from '../generation.store'

export const MAX_LORAS = 5

const DEFAULT_ASPECT_RATIO = AUTISMMIX_PONY_MODEL.aspectRatios[0]
const DEFAULT_RESOLUTION_TIER = AUTISMMIX_PONY_MODEL.resolutionTiers[1] // 1K
...
interface AutismmixPonyState {
  ...
  setOutputFormat: (format: AutismmixPonyState['outputFormat']) => void
  ...
  loadFromJob: (job: AutismmixPonyJob, options?: { includeSeed?: boolean }) => void
  resetForm: () => void
}
...
const DEFAULTS = {
  ...
  steps: AUTISMMIX_PONY_MODEL.defaultSteps,
  cfgScale: AUTISMMIX_PONY_MODEL.defaultCFGScale,
...
export const useAutismmixPonyStore = create<AutismmixPonyState>((set, get) => ({
  ...DEFAULTS,

  setField: (field, value) => set({ [field]: value } as never),
  setAspectRatio: (label) => {
    const ratio =
      AUTISMMIX_PONY_MODEL.aspectRatios.find((r) => r.label === label) ?? AUTISMMIX_PONY_MODEL.aspectRatios[0]
    const tier =
      AUTISMMIX_PONY_MODEL.resolutionTiers.find((t) => t.label === get().resolutionTierLabel) ??
      AUTISMMIX_PONY_MODEL.resolutionTiers[0]
    const { width, height } = resolveDimensions(AUTISMMIX_PONY_MODEL, ratio.ratio, tier)
    set({ aspectRatioLabel: label, width, height })
  },
  setResolutionTier: (label) => {
    const tier =
      AUTISMMIX_PONY_MODEL.resolutionTiers.find((t) => t.label === label) ?? AUTISMMIX_PONY_MODEL.resolutionTiers[0]
    const ratio =
      AUTISMMIX_PONY_MODEL.aspectRatios.find((r) => r.label === get().aspectRatioLabel) ??
      AUTISMMIX_PONY_MODEL.aspectRatios[0]
    const { width, height } = resolveDimensions(AUTISMMIX_PONY_MODEL, ratio.ratio, tier)
  ...
  loadFromJob: (job, options) => {
    const ratio = findClosestAspectRatio(AUTISMMIX_PONY_MODEL, job.width, job.height)
    const tier = findClosestResolutionTier(AUTISMMIX_PONY_MODEL, job.width, job.height)
    set({
