...
import type { IllustriousJob } from '../generation.store'

export const MAX_LORAS = 5

const DEFAULT_ASPECT_RATIO = ILLUSTRIOUS_MODEL.aspectRatios[0]
const DEFAULT_RESOLUTION_TIER = ILLUSTRIOUS_MODEL.resolutionTiers[1] // 1K
...
interface IllustriousState {
  ...
  setOutputFormat: (format: IllustriousState['outputFormat']) => void
  ...
  loadFromJob: (job: IllustriousJob, options?: { includeSeed?: boolean }) => void
  resetForm: () => void
}
...
const DEFAULTS = {
  ...
  steps: ILLUSTRIOUS_MODEL.defaultSteps,
  cfgScale: ILLUSTRIOUS_MODEL.defaultCFGScale,
...
export const useIllustriousStore = create<IllustriousState>((set, get) => ({
  ...DEFAULTS,

  setField: (field, value) => set({ [field]: value } as never),
  setAspectRatio: (label) => {
    const ratio = ILLUSTRIOUS_MODEL.aspectRatios.find((r) => r.label === label) ?? ILLUSTRIOUS_MODEL.aspectRatios[0]
    const tier =
      ILLUSTRIOUS_MODEL.resolutionTiers.find((t) => t.label === get().resolutionTierLabel) ??
      ILLUSTRIOUS_MODEL.resolutionTiers[0]
    const { width, height } = resolveDimensions(ILLUSTRIOUS_MODEL, ratio.ratio, tier)
    set({ aspectRatioLabel: label, width, height })
  },
  setResolutionTier: (label) => {
    const tier = ILLUSTRIOUS_MODEL.resolutionTiers.find((t) => t.label === label) ?? ILLUSTRIOUS_MODEL.resolutionTiers[0]
    const ratio =
      ILLUSTRIOUS_MODEL.aspectRatios.find((r) => r.label === get().aspectRatioLabel) ?? ILLUSTRIOUS_MODEL.aspectRatios[0]
    const { width, height } = resolveDimensions(ILLUSTRIOUS_MODEL, ratio.ratio, tier)
  ...
  loadFromJob: (job, options) => {
    const ratio = findClosestAspectRatio(ILLUSTRIOUS_MODEL, job.width, job.height)
    const tier = findClosestResolutionTier(ILLUSTRIOUS_MODEL, job.width, job.height)
    set({
